...
import { APP_VERSION, SYNC_STATE_FILE, DOWNLOAD_TEMP_EXTENSION, DOWNLOAD_CACHE_FOLDER_NAME } from '../../shared/constants';
...
import { executeSyncPlan, requestSyncCancel } from '../services/syncExecutorService';
...
import { normalizeUserError, setCancelRequested } from '../services/errorNormalizeService';

// Utilities
import { openFolderInExplorer } from '../utils/fileSystem';
import { normalizePath, getCacheDir } from '../utils/safePath';
...
export function registerIpcHandlers(): void {
  ...
  ipcMain.handle('sync:execute', async (event, syncPlan: SyncPlan) => {
    ...
    try {
      ...
      const userError = normalizeUserError(err);
      // Attach user-friendly info to the error for the renderer
      err.userFriendlyError = userError;
      throw err;
    }
  });
  ...
  ipcMain.handle('sync:cancel', async () => {
    try {
      ...
      log('error', `sync:cancel failed: ${err.message}`);
      throw err;
    }
  });
  ...
  ipcMain.handle('cache:clear', async () => {
    try {
      ...
      const modsPath = normalizePath(settings.lastModsPath);
      const cacheDir = getCacheDir(modsPath);

      // Delete the entire cache directory
      try {
        await fs.rm(cacheDir, { recursive: true, force: true });
        log('info', `Download cache cleared: ${cacheDir}`);
      } catch (err: any) {
        if (err.code !== 'ENOENT') {
          throw err;
        }
        // Cache dir didn't exist, that's fine
        log('info', 'No download cache to clear');
      }
    } catch (err: any) {
      log('error', `cache:clear failed: ${err.message}`);
      throw err;
    }
  });
  ...
  ipcMain.handle('recovery:perform', async () => {
    try {
      ...
      // 1. Delete any leftover .download temp files in mods root
      ...
      // 2. Also clean .download temp files in the cache directory (keep .jar cache files)
      try {
        const cacheTempFiles = await listFiles(cacheDir, DOWNLOAD_TEMP_EXTENSION);
        for (const tempFile of cacheTempFiles) {
          try {
            await deleteFile(tempFile);
            log('info', `Recovery: deleted cache temp file ${path.basename(tempFile)}`);
          } catch (err: any) {
            log('warn', `Recovery: could not delete cache temp file ${path.basename(tempFile)}: ${err.message}`);
          }
        }
      } catch (err: any) {
        // Cache dir might not exist, that's fine
        log('debug', `Recovery: cache dir not found or empty: ${err.message}`);
      }

      // 3. Remove the sync state file
