...
import { BACKUP_FOLDER_NAME, DISABLED_FOLDER_NAME, DOWNLOAD_CACHE_FOLDER_NAME, MOD_FILE_EXTENSION } from '../../shared/constants';
...
export async function scanModsDirectory(modsPath: string): Promise<LocalModFile[]> {
  ...
  const cacheDir = path.join(resolvedModsPath, DOWNLOAD_CACHE_FOLDER_NAME);

  // Filter out files that are in backup, disabled, or cache directories
  const jarFiles = allFiles.filter((filePath) => {
    const parentDir = path.dirname(filePath);
    if (parentDir === backupDir || parentDir === disabledDir || parentDir === cacheDir) {
      return false;
    }
    // Also skip if parent is a subfolder of backup, disabled, or cache
    if (parentDir.startsWith(backupDir + path.sep) || parentDir.startsWith(disabledDir + path.sep) || parentDir.startsWith(cacheDir + path.sep)) {
      return false;
    }
    return true;
  });
